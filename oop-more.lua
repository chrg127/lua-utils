...
function oop.new(class, ...)
    local mt = {
        ...
        __name = class.__classname and "instance of " .. class.__classname
                                   or "instance of unknown class"
    }
    local inst = setmetatable({ __is_instance = true }, mt)
...
function Object:extend(name, obj)
    ...
    local mt = {
        __index = self,
        __call = oop.new,
        __name = "instance of " .. self.__classname
    }
    return setmetatable(obj, mt)
end

function Object:define_operator(name, fn)
    print("defining operator " .. name .. " on " .. tostring(self))
    getmetatable(self)[name] = fn
    fmt.pyprint("mt = ", getmetatable(self))
end
...
function Object:super()
    local class = self.__is_instance and getmetatable(self).__index or self
...
function oop.is_instance(obj, class)
    while obj ~= nil do
        local superclass = getmetatable(obj).__index
